...
def twitter_posts(hashtags):
    ...
    for tag in hashtags:
        ...
        for i, t in tqdm(enumerate(scraper.get_items()), total = no_tweets, desc = tag):
            data = ' '.join(re.sub("(@[A-Za-z0-9]+)|([^0-9A-Za-z \t])|(\w+:\/\/\S+)"," ",t.content).split())
...
jbl.dump(model_eng, 'eng_model.pkl')
jbl.dump(vectorizer, 'eng_vectorizer.pkl')
jbl.dump(lsa, 'eng_lsa.pkl')
jbl.dump(labelencoder_name_mapping, 'eng_labelencoder_name_mapping.pkl')